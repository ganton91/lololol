...
- Progress:
  - Implemented in the current slice:
    ...
    - As the current deeper alignment experiment, edge-based `Align` now reads its authoritative direction from the snapped source segment itself, uses the drag only to choose the direction sign, materializes candidate/persistent dynamic families from that normalized base vector instead of from an angle-quantized representation, and derives the runtime angle display/transform fields from those same vector-authored step coefficients.
