...
- The canvas render stack now keeps always-visible terracotta `World` axes underneath everything, shows the draft canvas (`Grid` + draft-plane axes) only in `Draw`, and renders the full active layer above that draft canvas so the current drafting target stays visually on top.
